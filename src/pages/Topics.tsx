...
import { useTopics, useCreateTopic, useDeleteTopic, useUpdateTopic } from '../hooks/useTopics';
...
export default function Topics() {
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    createTopic.mutate(
      { name: newName.trim(), category_id: newCategoryId },
      {
        onSuccess: () => {
          setIsCreating(false);
          setNewName('');
  ...
  const handleUpdate = (id: string) => {
    ...
    updateTopic.mutate(
      { id, name: editName.trim(), category_id: editCategoryId },
      {
        onSuccess: () => {
          setEditingId(null);
          setEditName('');
  ...
  if (loadingTopics || loadingCategories) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="font-display text-4xl font-bold text-on-surface mb-2 tracking-tight">Topics</h1>
          <p className="text-lg text-secondary">Manage topics within your categories.</p>
        </div>
        <button
          ...
          <form onSubmit={handleCreate} className="flex flex-col sm:flex-row items-center gap-4">
            ...
            >
              ...
              {categories?.map((s: any) => (
                <option key={s.id} value={s.id}>{s.projects?.name} &gt; {s.name}</option>
              ))}
            </select>
            <input
              type="text"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {topics?.length === 0 ? (
          ...
          topics?.map((topic: any) => (
            <article key={topic.id} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 anti-gravity-hover flex flex-col group">
              <div className="flex justify-between items-start mb-4">
                <div className="bg-surface-container-low px-3 py-1 rounded-full text-xs font-semibold text-secondary inline-flex items-center gap-1 border border-outline-variant/50">
                  ...
                  {topic.categories?.projects?.name || 'Unknown Project'} &gt; {topic.categories?.name || 'Unknown Category'}
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={() => {
                      if (confirm('Are you sure you want to delete this topic? All associated study sessions and revisions will be deleted.')) {
              ...
              {editingId === topic.id ? (
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-full bg-surface border border-outline-variant rounded px-2 py-1 text-sm text-on-surface focus:outline-none focus:ring-1 focus:ring-primary-container"
                    />
                    <button onClick={() => handleUpdate(topic.id)} className="text-primary-container p-1">
                ...
                <>
                  <h3 className="font-display text-xl font-bold text-on-background mb-2">{topic.name}</h3>
